...
TableElement _newPhone() {
  final phone = Phone.defaultPhone();
  return phone.toInputTable(PhoneOptions.fromPhone(phone, 'new'));
}
...
List<TableRowElement> accountEditTableRows(Account account) {
  ...
  addressRow.children.add(Element.td()
    ..children.addAll([
      DivElement()
        ..id = 'addresses'
        ..children.addAll(account.addresses.map((address) => address.data.toInputTable(AddressOptions.fromAddress(address.data)))),
      DivElement()
  ...
  phoneRow.children.add(Element.td()
    ..children.addAll([
      DivElement()
        ..id = 'phones'
        ..children.addAll(account.phones.map((phone) => phone.data.toInputTable(PhoneOptions.fromPhone(
              phone.data,
              phone.id.toString(),
            )))),
      DivElement()
        ..children.add(ButtonElement()
          ...
          ..onClick.listen(
            (event) => querySelector('#phones')?.children.add(_newPhone()),
          )),
    ]));
  rows.add(phoneRow);
